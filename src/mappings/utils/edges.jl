...
########################################################################
"""
    _eccentricity_tolerance(a,rc,tole)

tweak the constant eccentricity tolerance `tole` according to the semi-major axis.

For low semimajor axis `a` compared to the system's characteristic size `rc`, increased
tolerance on eccentricity `e` to the domain borders for interpolations.

We effectively want to switch at ``a*e > rc * tole``, i.e., ``tole = rc * tole / a``
...
"""
_eccentricity_tolerance(a,rc,tole) = min(1/3, max(tole, tole*rc/a))
...
"""
    _interpolate_edges_ae(fun, a, e, params)
